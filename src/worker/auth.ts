...
/** The read-only public demo account. */
export const isDemo = (env: Env, email: string | null | undefined) =>
  !!email &&
  !!env.DEMO_EMAIL &&
  email.toLowerCase() === env.DEMO_EMAIL.toLowerCase();
...
export async function isEmailAllowed(
  ...
  if (isAdmin(env, e) || (env.DEMO_EMAIL && e === env.DEMO_EMAIL.toLowerCase()))
    return true;
...
export async function verifyMagicToken(
  ...
  const row = await env.DB.prepare(
    "SELECT email, expires_at FROM auth_tokens WHERE token = ?",
  )
    .bind(token)
