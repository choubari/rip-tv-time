 ...
 * `key` should identify the actor + action (e.g. `search:<userId>`).
 */
...
export async function seedImport(
  ...
  userId: string,
  data: ParsedImport,
): Promise<{ titles: number; episodes: number }> {
  const p = data.profile;
  if (p.name || p.bio || p.cover_url || p.avatar_url) {
    await env.DB.prepare(
      "UPDATE users SET name = COALESCE(?, name), bio = COALESCE(?, bio), cover_url = COALESCE(?, cover_url), avatar_url = COALESCE(?, avatar_url) WHERE id = ?",
    )
      .bind(
        p.name ?? null,
        p.bio ?? null,
  ...
  await env.DB.batch([
    env.DB.prepare(
      "DELETE FROM list_items WHERE list_id IN (SELECT id FROM lists WHERE user_id = ?)",
    ).bind(userId),
    env.DB.prepare("DELETE FROM lists WHERE user_id = ?").bind(userId),
    env.DB.prepare("DELETE FROM watched_episodes WHERE user_id = ?").bind(
      userId,
    ),
  ...
  for (const list of data.lists) {
    ...
    list.items.forEach((it, i) => {
      ...
      stmts.push(
        ...
        ).bind(id, it.kind, it.tvdb_id, it.imdb_id, it.name),
      );
      stmts.push(
        env.DB.prepare(
 ...
 */
export async function refreshNewEpisodes(
  env: Env,
  limit = 60,
  ...
  for (const row of results) {
    ...
    if (lastKey > (row.max_watched ?? 0)) {
      await env.DB.prepare(
        "UPDATE library SET status = 'watching' WHERE user_id = ? AND title_id = ?",
      )
        .bind(row.user_id, row.title_id)
        .run();
      updated++;
    }
  }
...
export async function refOf(env: Env, titleId: string): Promise<number> {
  const r = await env.DB.prepare("SELECT rowid AS ref FROM titles WHERE id = ?")
    .bind(titleId)
    .first<{ ref: number }>();
  return r?.ref ?? 0;
}
...
export async function resolveBatch(
  ...
  for (const row of results) {
    ...
    if (meta && meta.tmdb_id > 0) {
      ...
      await env.DB.prepare("UPDATE titles SET resolve_failed = 1 WHERE id = ?")
        .bind(row.id)
        .run();
    }
  }
...
async function dedupeByTmdb(env: Env): Promise<void> {
  ...
  for (const d of dupes) {
    const { results: others } = await env.DB.prepare(
      ...
      .bind(d.kind, d.tmdb_id, d.keep)
      .all<{ id: string }>();
    for (const o of others) {
      await env.DB.batch([
        // Move any library/episode/list rows to the canonical id, then drop the dup.
        env.DB.prepare(
          "UPDATE OR IGNORE library SET title_id = ? WHERE title_id = ?",
        ).bind(d.keep, o.id),
        env.DB.prepare(
          "UPDATE OR IGNORE watched_episodes SET title_id = ? WHERE title_id = ?",
        ).bind(d.keep, o.id),
        env.DB.prepare(
          "UPDATE OR IGNORE list_items SET title_id = ? WHERE title_id = ?",
        ).bind(d.keep, o.id),
        env.DB.prepare("DELETE FROM library WHERE title_id = ?").bind(o.id),
        env.DB.prepare("DELETE FROM watched_episodes WHERE title_id = ?").bind(
          o.id,
        ),
...
function rowToMeta(r: any): TitleMeta {
  return {
    ...
    ref: r.ref ?? r.rowid ?? 0,
    kind: r.kind,
    tmdb_id: r.tmdb_id === -1 ? null : r.tmdb_id,
    imdb_id: r.imdb_id,
    tvdb_id: r.tvdb_id,
    name: r.name,
...
function rowToItem(r: any): LibraryItem {
  const base = {
    ...
    total_episodes:
      r.stored_total && r.kind === "show"
        ? r.stored_total
        : rowToMeta(r).total_episodes,
  };
...
export async function getLibrary(
  ...
  const stmt = env.DB.prepare(
    `SELECT t.*, t.rowid AS ref, l.status, l.is_favorite, l.rating, l.added_at, l.last_watched_at,
     ...
     ORDER BY COALESCE(l.last_watched_at, l.added_at) DESC NULLS LAST, t.rowid ASC`,
  );
  const { results } = await (
...
export async function getUnmatched(
  ...
  const { results } = await env.DB.prepare(
    ...
    .bind(userId)
    .all<{ ref: number; name: string; kind: string }>();
  return results;
}
...
export async function getLists(
  env: Env,
  userId: string,
): Promise<import("../../shared/types").ListSummary[]> {
  const { results: lists } = await env.DB.prepare(
    "SELECT id, name FROM lists WHERE user_id = ? ORDER BY id",
  )
    .bind(userId)
  ...
  for (const l of lists) {
    const { results } = await env.DB.prepare(
      `SELECT t.*, t.rowid AS ref, li.ordering, lib.status, lib.is_favorite, lib.rating, lib.added_at, lib.last_watched_at,
       ...
       WHERE li.list_id = ?
       ORDER BY COALESCE(lib.last_watched_at, lib.added_at) DESC NULLS LAST, t.rowid ASC`,
    )
...
export async function getTitle(
  ...
  titleId: string,
  byRef = false,
): Promise<(LibraryItem & { episodes: any[] }) | null> {
  const r = await env.DB.prepare(
    `SELECT t.*, t.rowid AS ref, l.status, l.is_favorite, l.rating, l.added_at, l.last_watched_at
  ...
  const { results: episodes } = await env.DB.prepare(
    ...
    .bind(userId, titleId)
    .all<{ season: number; watched: number }>();
  const regularWatched = episodes.filter(
    (e) => e.season > 0 && e.watched,
  ).length;
  const storedTotal = episodes.filter((e) => e.season > 0).length;
  const item = rowToItem({
    ...r,
    episodes_watched: regularWatched,
    stored_total: storedTotal,
  });
  return {
    ...item,
    tracked: r.status != null,
    episodes: episodes.map((e) => ({ ...e, watched: !!e.watched })),
  };
}

export async function getStats(env: Env, userId: string): Promise<Stats> {
  const q = async (sql: string) =>
    (await env.DB.prepare(sql).bind(userId).first<any>()) ?? {};
  const shows = await q(
    "SELECT COUNT(*) c FROM library WHERE user_id = ? AND kind = 'show'",
  );
  const movies = await q(
    "SELECT COUNT(*) c FROM library WHERE user_id = ? AND kind = 'movie'",
  );
  const eps = await q(
    "SELECT COUNT(*) c FROM watched_episodes WHERE user_id = ? AND season > 0 AND watched = 1",
  );
  const moviesWatched = await q(
    "SELECT COUNT(*) c FROM library WHERE user_id = ? AND kind='movie' AND status='finished'",
  );
  ...
  const tv = await q(
    "SELECT COALESCE(SUM(COALESCE(w.runtime, t.runtime, 40)),0) m FROM watched_episodes w JOIN titles t ON t.id=w.title_id WHERE w.user_id = ? AND w.season > 0 AND w.watched = 1",
  );
  const mv = await q(
    "SELECT COALESCE(SUM(COALESCE(t.runtime,100)),0) m FROM library l JOIN titles t ON t.id=l.title_id WHERE l.user_id = ? AND l.kind='movie' AND l.status='finished'",
  );
  return {
    shows: shows.c,
    movies: movies.c,
    episodes_watched: eps.c,
    movies_watched: moviesWatched.c,
    tv_minutes: tv.m,
    movie_minutes: mv.m,
  };
}
...
export async function ensureTitle(
  ...
  const existing = await env.DB.prepare(
    "SELECT id FROM titles WHERE kind = ? AND tmdb_id = ?",
  )
    .bind(kind, tmdbId)
    .first<{ id: string }>();
  if (existing) return existing.id;
  const id = `tmdb:${kind}:${tmdbId}`;
  const exists = await env.DB.prepare("SELECT 1 FROM titles WHERE id = ?")
    .bind(id)
    .first();
  if (exists) return id;
  const meta = await fetchDetail(await tmdbKey(env), kind, tmdbId);
...
export async function updateLibrary(
  ...
  userId: string,
  titleId: string,
  patch: { status?: Status; is_favorite?: boolean; rating?: number | null },
) {
  const sets: string[] = [];
  const binds: any[] = [];
  ...
  if (patch.is_favorite !== undefined) {
    ...
    binds.push(patch.is_favorite ? 1 : 0);
  }
  if (patch.rating !== undefined) {
    sets.push("rating = ?");
    binds.push(patch.rating);
  }
  if (!sets.length) return;
  await ensureLibraryRow(env, userId, titleId);
  binds.push(userId, titleId);
  await env.DB.prepare(
    `UPDATE library SET ${sets.join(", ")} WHERE user_id = ? AND title_id = ?`,
  )
    .bind(...binds)
    .run();
}
...
export async function toggleEpisode(
  ...
  await env.DB.prepare(
    ...
    .bind(userId, titleId, season, episode, watched ? 1 : 0)
    .run();
  if (watched)
    await env.DB.prepare(
      "UPDATE library SET last_watched_at = datetime('now') WHERE user_id = ? AND title_id = ?",
    )
      .bind(userId, titleId)
      .run();
}
